...
- The dashboard `Start Service` button starts a low-latency hidden camera/microphone capture path that records overlapping 2-second media segments every 500ms and runs a 100ms model-monitor scheduler. Model requests can overlap up to a small dispatch cap so vLLM stays focused on recent windows instead of building a stale queue; the dashboard keeps the newest completed request visible.
- The model monitor currently sends the latest freshly completed rolling 2-second webcam `video/mp4` data URL, muxes audio into the MP4, and enables `use_audio_in_video`, matching the working standalone audio/video test as closely as possible while screen media is held out for isolation. It displays only the latest paired model video/response in the dashboard without executing actions yet.
- The monitor only submits a newly completed camera clip once and skips clips whose end time is older than 1.5 seconds at request time; skipped scheduler ticks mean the next media segment has not closed yet, the latest segment is too stale for live mode, or the model dispatch cap is full.
